...
async def download_short(
    ...
    last_error: str = ""
    for fmt in _SHORTS_FORMAT_PRIORITIES:
        tmp_path = None
        try:
            ...
            if process.returncode != 0:
                ...
                logger.debug(f"[YouTube] Shorts format '{fmt}' failed: {last_error}")
                continue

            if not os.path.exists(tmp_path) or os.path.getsize(tmp_path) == 0:
                logger.debug(f"[YouTube] Shorts format '{fmt}' produced empty file")
                continue
            ...
            logger.info(
                f"[YouTube] Downloaded Short: {file_size / 1024 / 1024:.1f} MB (format: {fmt})"
            )
            ...
            last_error = str(e)
            logger.debug(f"[YouTube] Shorts format '{fmt}' exception: {e}")
            continue
        finally:
    ...
    raise DownloadError(
        f"All Shorts format attempts failed. Last error: {last_error}",
        platform="youtube",
    )
